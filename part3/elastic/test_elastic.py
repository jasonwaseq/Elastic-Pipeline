...
@pytest.mark.parametrize("datapath_reset_p", [0, 1])
@pytest.mark.parametrize("datapath_gate_p", [0, 1])
@pytest.mark.parametrize("test_name", tests)
@pytest.mark.parametrize("simulator", ["verilator", "icarus"])
@max_score(0)
def test_each(test_name, simulator, width_p, datapath_reset_p, datapath_gate_p):
...
@pytest.mark.parametrize("simulator", ["verilator", "icarus"])
@max_score(1)
def test_all(simulator, width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
...
@max_score(.4)
def test_lint(simulator, width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
...
@pytest.mark.parametrize("simulator", ["verilator"])
@max_score(.1)
def test_style(simulator, width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
    del parameters['simulator']
...
async def datapath_reset_test(dut):
    """Test for datapath_reset_p"""

    depth_p = 1
    l = depth_p
    rate = 1
    value = (1 << dut.width_p.value) - 1
    ...
    if(dut.datapath_reset_p.value == 1):
        assert dut.data_o == 0,  f"Error! Value on data_o should be zero after reset when datapath_reset_p == 1. Expected: 0. Got: {dut.data_o.value}"
    else:
        assert dut.data_o == value,  f"Error! Value on data_o should not be zero after reset when datapath_reset_p == 0. Expected: {value}. Got: {dut.data_o.value}"

@cocotb.test()
async def datapath_gate_test(dut):
    """Test for datapath_gate_p"""
    ...
    if(dut.datapath_gate_p.value == 1):
        assert dut.data_o == prior,  f"Error! Value on data_o should not be updated when ready_o is high, when datapath_gate_p == 1. Expected: {prior.value}. Got: {dut.data_o.value}"
    else:
        assert dut.data_o == value,  f"Error! Value on data_o should not be updated when ready_o is high, when datapath_gate_p == 0. Expected: {value}. Got: {dut.data_o.value}"
...
async def fill_test(dut):
    ...
    if(not success):
        assert nconsumed != depth_p, f"Error! Could not fill fifo with {depth_p} elements in {depth_p} cycles. Elastic consumed {nconsumed} elements."
...
async def fill_empty_test(dut):
    ...
    if(not success):
        assert nproduced != depth_p, f"Error! Could not empty fifo with {depth_p} elements in {depth_p} cycles. Elastic produced {nproduced} elements."
...
async def in_fuzz_test(dut):
    """Transmit 100 random data elements at 50% line rate (Input/Producer is fuzzed)"""
...
async def inout_fuzz_test(dut):
    """Transmit 100 random data elements at ~25% line rate (Both are fuzzed)"""
...
async def stream_test(dut):
    """Transmit 100 random data elements at 100% line rate"""
